...
/// </summary>
public class ComprehensiveApiIntegrationTests : IClassFixture<ApiWebApplicationFactory>
{
    ...
    public ComprehensiveApiIntegrationTests(ApiWebApplicationFactory factory, ITestOutputHelper output)
    {
        _factory = factory;
    ...
    {
        // Act - Updated to use Indicator terminology
    ...
    [Fact]
    public async Task GetIndicators_WithValidAuthentication_ShouldReturnSuccess()
    {
        // Arrange
        ...
        _output.WriteLine($"Indicators Response: {content}");
    }

    [Fact]
    public async Task GetIndicators_WithPagination_ShouldReturnPaginatedResults()
    {
        // Arrange
        var token = await GetValidJwtTokenAsync();
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        ...
        // Act - Updated to use Indicator terminology
        var response = await _client.GetAsync("/api/indicator?page=1&pageSize=5");
        var content = await response.Content.ReadAsStringAsync();
        ...
        _output.WriteLine($"Paginated Indicators Response: {content}");
    }

    [Fact]
    public async Task ExecuteIndicator_WithValidId_ShouldReturnExecutionResult()
    {
        // Arrange
        var token = await GetValidJwtTokenAsync();
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        ...
        // Act - Updated to use Indicator terminology
        var response = await _client.PostAsync("/api/indicator/1/execute", jsonContent);
        var content = await response.Content.ReadAsStringAsync();
        ...
        _output.WriteLine($"Execute Indicator Response ({response.StatusCode}): {content}");
    }
    ...
    public async Task RateLimit_ExcessiveRequests_ShouldReturnTooManyRequests()
    {
        ...
        {
            tasks.Add(_client.GetAsync("/api/indicator"));
        }
    ...
    /// </summary>
    private Task<string> GetValidJwtTokenAsync()
    {
        ...
        return Task.FromResult("test-jwt-token-for-integration-tests");
    }
}

/// <summary>
/// Custom WebApplicationFactory for integration testing
/// Enhanced from MonitoringGrid.Tests project
/// </summary>
