...
export const UltimateDataTableDemo: React.FC = () => {
  ...
  const [tableData, setTableData] = useState(() => {
    ...
    const teams = ['DevOps Team', 'Database Team', 'API Team', 'Security Team', 'Network Team', 'Infrastructure Team'];
    ...
    return Array.from({ length: 50 }, (_, index) => ({
      ...
      lastRun: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16).replace('T', ' '),
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTableData(prev => prev.map(item => ({
        ...
        responseTime: Math.max(10, Math.min(1000, item.responseTime + Math.floor((Math.random() - 0.5) * 50))),
    ...
    <Box sx={{ p: 4 }}>
      ...
      <Box sx={{ mb: 4, textAlign: 'center' }}>
        ...
        <Typography variant="h6" color="text.secondary">
          Enterprise-grade data table with advanced features, real-time updates, and professional styling
        </Typography>
      </Box>
